...
const ProfileManager: React.FC<Props> = ({ permaweb }) => {
  ...
  const renderImage = (src: string, alt: string, size = "w-20 h-20") => {
    if (!src)
      return (
        <div className={`bg-gray-200 ${size} flex items-center justify-center rounded`}>
          No {alt}
        </div>
      );
      ...
      <div className="space-y-4 bg-gray-50 p-4 rounded-xl border border-gray-100">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <button
              onClick={fetchById}
              className="bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded-lg font-semibold w-full mt-2 transition"
            >
              Fetch by Profile ID
            </button>
          </div>
          <div>
            ...
            />
            <button
              onClick={fetchByWallet}
              className="bg-purple-500 hover:bg-purple-600 text-white px-4 py-2 rounded-lg font-semibold w-full mt-2 transition"
            >
              Fetch by Wallet
            </button>
          </div>
      ...
      {profile && (
        <div className="p-6 border border-gray-200 rounded-2xl bg-gradient-to-br from-gray-50 to-white shadow-inner mt-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
              <p className="mb-1">
                <span className="font-semibold text-gray-700">ID:</span> {profile.id}
              </p>
              <p className="mb-1">
              ...
              <p className="mb-1">
                <span className="font-semibold text-gray-700">Wallet:</span> {profile.walletAddress}
              </p>
            </div>
              ...
              <ul className="list-disc pl-6 space-y-1 text-gray-700">
                {profile.assets.map((asset) => (
                  <li key={asset.id} className="bg-gray-100 rounded px-2 py-1">
                    ...
                    <span className="font-semibold">Qty:</span> {asset.quantity},{" "}
